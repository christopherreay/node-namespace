atRoot.regex.startsWithToReturn = /^\s*toReturn\s*=/;
      ...
      this.namespace.defaultDict = this.namespace;
      ...
      this.namespace.NotFound = {"namespaceFunctionConstant": "NotFound"};
      Object.freeze(this.namespace.NotFound);
			this.namespace.isNotFound = function(value, address)
      {
      ...
      this.namespace.traverse = function(cursor)
      {
        ...
        { let errorMessage = "@: ERROR: atSrc: namespace: getIfExists: object : this Object is not a valid namespace root object";
          atRoot.ls (errorMessage, { "object": util.inspect(object).slice(200), "address": address} )
          throw new Error(errorMessage);
        }
      ...
      this.namespace.getIfExists_callback = function(object, address, found, notFound)
      ...
      { const toReturn = atRoot.namespace.getIfExists(object, address);
        ...
        return toReturn;
			...
			this.namespace.getMustExistRM = function(object, address)
			{ try
      ...
      this.namespace.destructureMustExist = function(object, destructureDefinition, options)
      {
        let toReturn = {}
      ...
      // this.namespace.toEval_destructure = function(object, varDestructure)
      // {
      //   let toEval_list = [];

      // }
      this.namespace.allMustExist = function(object, checkDefinition, options={}, toReturnRoot={})
      {

        if (atRoot.isString(checkDefinition))
        {
          let key = checkDefinition;
          // let realPropertyName = checkDefinition;
          // if (checkDefinition.startsWith("@"))  realPropertyName = checkDefinition.replace(/\@/,"");
          let noDotCheckDefinition = key.replace(".","DOT");

          let targetValue = atRoot.namespace.getMustExist(object, key, options);
          // if (options.keepAtVars !== true || checkDefinition.startsWith("@") ) toReturnRoot[noDotCheckDefinition] = targetValue;
          toReturnRoot[noDotCheckDefinition] = targetValue;
        }
        else if (atRoot.isArray(checkDefinition))
        {
          for (let item of checkDefinition)
          {
            atRoot.namespace.allMustExist(object, item, options, toReturnRoot);
          }
        }
        else if (atRoot.isObject_notArray(checkDefinition))
        {
          for (let [key, value] of Object.entries(checkDefinition))
          {
            // let realPropertyName = key;
            // if (key.startsWith("@"))  realPropertyName = key.replace(/\@/,"");

            let noDotKey = key.replace(".","DOT");
            
            let recurseObject = namespace.getMustExist(object, key, options);
            // if (options.keepAtVars !== true || key.startsWith("@") ) toReturnRoot[noDotKey] = recurseObject;
            toReturnRoot[noDotKey] = recurseObject;

            atRoot.namespace.allMustExist(recurseObject, value, options, toReturnRoot);            
          }
        }
        return toReturnRoot;
      };

      this.namespace.toEval_allMustExist = function()
      {
        let uniqueID = require("uuid").v4().replace(/\-/g,"");
        let toEval_toReturn = 
`
let options = toEval_allMustExist_params[2] || {};
Object.assign(options, {"keepAtVars":true});
let toReturnRoot_allMustExist_${uniqueID} = namespace.allMustExist(...toEval_allMustExist_params);


eval(namespace.toEval_allMustExist_helper(toReturnRoot_allMustExist_${uniqueID}, "${uniqueID}"));
`;
        return toEval_toReturn;
      };
      this.namespace.toEval_allMustExist_helper = function(allMustExist_returnValue, uniqueID)
      {
        let varsToDestructure = [];
        for (let varName of Object.keys(allMustExist_returnValue))
        {
          if (varName.startsWith("@")) varsToDestructure.push(varName.replace(/\@/, ""));
        }
        
        atRoot.ls("@: allMustExist_returnValue", allMustExist_returnValue);

        let toEval_toReturn =
`var { ${varsToDestructure.join(", ")} } = toReturnRoot_allMustExist_${uniqueID};
`;
        atRoot.ls(toEval_toReturn);
        return toEval_toReturn;
      }
      ...
      this.namespace.flatten = function(objectToFlatten, currentNamespace, toReturn)
      {
      ...
      this.namespace.freezeEverything = function(object, options, recurseRoot)
      {
        ...
        // if (recurseRoot !== false) 
        Object.freeze(object);

        return object;
      }
			this.namespace.extendObjectWithVennComplementOfTwo = function(object, vennData, optionsDict)
			{ 
			...
			this.namespace.venn = function(object1, object2, fullAddress, toReturn)
			{
...
//////////////////////// CONTINUE ///////////////////
      ...
      this.namespace.deepCopy = function(copyThisObject, fullDottedAddressString, runtimeNamespaceList) //, objectToReturn)
      {
      ...
      this.namespace.toEval_localVars_store = function(objectName, address, localVarNameList, localVarNameDict)
      {
        if (address === null)
        { addressString = "null";
        }
      ...
      this.namespace.toEval_localVars_load = function(objectName, address, localVarNameList, localVarNameDict)
      {
        if (address === null)
        { addressString = "null";
        }
      ...
      this.namespace.joinDot = function(...toJoin)
      { return atRoot.namespace.joinWordList_helper(".", ...toJoin);
      }
      this.namespace.joinNamespaceDots = this.namespace.joinDots;

      this.namespace.joinSlash = function(...toJoin)
      { return atRoot.namespace.joinWordList_helper("/", ...toJoin);
      }
      this.namespace.joinNamespaceSlash = this.namespace.joinSlash;

      this.namespace.joinWordList_helper = function(separator, ...toJoin)
      {
        ...
        this.namespace.compareSlice = function(namespaceToCheck, compareTo, sliceParams, options)
        {
          debugger;
          try
          {
      ...
      this.namespace.iterateFragment = 
          function* (namespaceToIterate, tweenWith)
          {
