...
module.exports = function handleRequest(context, request) {
  ...
  // ── preamble — establish and then assert every dependency ──
  namespace.setOrDefault(context, "config",                               loadConfigFromDisk());
  namespace.setOrDefault(context, "users." + userId_probed + ".entries",  []);

  const config_node      = namespace.getMustExist(context, "config");
  const userEntries_node = namespace.getMustExist(context, "users." + userId_probed + ".entries");
  // ── end preamble ───────────────────────────────────────────
